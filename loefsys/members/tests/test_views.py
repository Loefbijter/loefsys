...
class UserProfileViewTestCase(TestCase):
    ...
    def test_other_profiles_hide_private_contact_details(self):
        """Public profiles should not reveal private contact details."""
        owner = G(
            get_user_model(),
            first_name="Owner",
            last_name="User",
        ...
        viewer = G(get_user_model())
        self.client.force_login(viewer)

        response = self.client.get(
            reverse("members:profile", kwargs={"slug": owner.slug})
        )

        self.assertEqual(response.status_code, 200)
        self.assertNotContains(response, owner.email)
        self.assertNotContains(response, str(owner.phone_number))

    def test_profile_edit_birthday_placeholder_and_dutch_format(self):
        """Birthday input should guide users and accept dd-mm-yyyy format."""
        user = G(get_user_model())
        self.client.force_login(user)
        ...
        post_response = self.client.post(
            reverse("members:user-profile-edit"),
            {
                ...
                "pod_kb_link": user.pod_kb_link,
                "pod_zb_link": user.pod_zb_link,
                "gender": user.gender,
                "birthday": "12-05-1999",
        ...
        self.assertEqual(str(user.birthday), "1999-05-12")
